...
class RL_environment():
    ...
    def update_value(self, position, next_action = None):
        ...
        if self.mode == "Q-Learning":
            ...
            delta_error = self.reward_function[self.pos[0], self.pos[1]] + self.gamma * self.values[self.pos[0], self.pos[1], 4] - self.values[position[0], position[1], 4]
            self.values[position[0], position[1], z] += self.learning_rate * delta_error
            self.values[position[0], position[1], 4] += self.beta * delta_error
        else:
